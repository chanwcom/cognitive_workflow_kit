...
class RelativePositionEncoding(tf.keras.layers.Layer):
    ...
    def __init__(self, input_dim):
        ...
        self.inv_freq = tf.Variable(
            np.array(1 / (10000.0 ** (np.arange(0, input_dim, 2.0) / input_dim))),
            dtype=tf.float32,
            trainable=False
        )
    ...
    def call(self, seq_len):
        ...
        pos_embed = tf.concat([tf.math.sin(sinusoid_input),
...
def compute_weight_from_energy(energy, x_len, left_mask, right_mask,
                               stream=False, dropout=0.0,
                               valid_state=None,
    ...
    if x_len is None:
        ...
        if renormalize_dropout and dropout > 0.0:
            if dropout_column:
                drop_shape = (batch_size, num_heads, 1, num_keys // dropout_window + 1)
            else:
                drop_shape = (batch_size, num_heads, num_querys, num_keys // dropout_window + 1)
            ...
            seq_mask = tf.math.logical_and(seq_mask, keep_mask[:, :, :, :num_keys])
        seq_mask = tf.cast(seq_mask, tf.float32)

    if stream:
        assert right_mask == 0
        if need_tile:
            seq_mask = tf.broadcast_to(seq_mask, [batch_size, num_heads, num_querys, num_keys])
        ...
        mask_left_inverse = tf.sequence_mask(tf.range(num_querys),
            maxlen=num_keys, dtype=tf.float32)
        mask = tf.multiply(seq_mask, mask_right - mask_left_inverse)

        if valid_state is not None:
            mask = tf.concat([tf.zeros([tf.math.maximum(0, left_mask - valid_state)]),
                              tf.ones([tf.math.minimum(valid_state + 1, left_mask + 1)])], 0) * mask
...
def unstack_and_matmul(mat1, mat2, transpose_b=False):
    ...
    result = []
    for sub_mat1, sub_mat2 in _match_dim_zip(tf.unstack(mat1), tf.unstack(mat2)):
        result.append(unstack_and_matmul(sub_mat1, sub_mat2, transpose_b=transpose_b))

    return tf.stack(result)

def einsum(mat1, mat2, trans1=None, trans2=None, trans3=None, transpose_b=False):
...
def compute_context(att_weight, value, stream=False, transpose_value=False):
    ...
    if len(att_weight.shape) == 3 and value.shape[0] == 1:
        ...
        context = einsum(att_weight, value,
        ...
        equation = 'bnij,{}->bind'.format('bndj' if transpose_value else 'bjnd')
...
class RelativeAttention(tf.keras.layers.Layer):
    ...
    def __init__(self, att_dim, att_head, input_dim,
            left_mask=-1, right_mask=-1, dropout=0.0,
            renormalize_dropout=False,
            dropout_window=1, dropout_column=False):
        super().__init__()
        self.num_head = att_head
        self.content_key = tf.keras.layers.Dense(att_dim, use_bias=False) # W_k,E
        self.relative_key = tf.keras.layers.Dense(att_dim, use_bias=False) # W_k,R
    ...
    def call(self, x, x_len, training=False):
        ...
        q_con = tf.reshape(q_con, [B, S, self.num_head, self.head_dim]) # [B,Q,N,D]
        q_rel = self.relative_query(x) # E_x_i' * W_q' + v'
        q_rel = tf.reshape(q_rel, [B, S, self.num_head, self.head_dim]) # [B,Q,N,D]

        k_con = self.content_key(x) # W_k,E * E_x_j
        k_con = tf.reshape(k_con, [B, S, self.num_head, self.head_dim]) # [B,Q,N,D]
        if q_con.shape[0] is not None:
            content_energy = einsum(q_con, k_con, trans1=[0,2,1,3], trans2=[0,2,3,1])
        else:
            content_energy = tf.einsum('bind,bjnd->bnij', q_con, k_con) # [B,N,Q,K]

        k_rel = self.relative_key(self.R(S)) # [2*K-1, N*D]
        rel_pos_len = self.R.pos_emb_length(S)
        k_rel = tf.reshape(k_rel, [rel_pos_len, self.num_head, self.head_dim]) # [2*K-1, N, D]

        if q_rel.shape[0] is not None:
            relative_energy = einsum(q_rel, tf.expand_dims(k_rel, axis=0),
                                     trans1=[0,2,1,3], trans2=[0,2,3,1])
        else:
            relative_energy = tf.einsum('bind,jnd->bnij', q_rel, k_rel) # [B,N,Q,2*K-1]
        relative_energy = self.rel_shift(relative_energy) # [B,N,Q,2*K-1] -> [B, N, Q, K]

        energy = tf.divide(content_energy + relative_energy,
                           tf.math.sqrt(tf.cast(self.head_dim, tf.float32))) # [B,N,Q,K]

        dropout = self.dropout if training else 0
        attention = compute_weight_from_energy(energy, x_len, self.left_mask, self.right_mask,
                        dropout=dropout, renormalize_dropout=self.renormalize_dropout,
                        dropout_window=self.dropout_window, dropout_column=self.dropout_column)

        v = tf.reshape(self.value(x), [B, S, self.num_head, self.head_dim]) # [B,K,N,D]
...
class SelfAttention(tf.keras.layers.Layer):
    ...
    def __init__(self, att_dim, att_head, use_bias=True,
            left_mask=-1, right_mask=-1, use_proj=False,
            dropout=0.0, renormalize_dropout=False,
            dropout_window=1, dropout_column=False):
        super().__init__()
        self.num_head = att_head
        self.content_key = tf.keras.layers.Dense(
            att_dim, use_bias=use_bias)  # W_k,E
        self.content_query = tf.keras.layers.Dense(att_dim,
    ...
    def call(self, x, x_len, training=True, stream=False, past_state=None, valid_state=None):
        """
        x: [batch, seq, H]
        i: query ([0, seq-1])
        j: key ([0, seq-1])
        ...
        if stream:
            ...
            past_key, past_value = tf.split(
                past_state, [self.left_mask, self.left_mask], axis=1)
        ...
        energy = tf.divide(content_energy,
                           tf.math.sqrt(tf.cast(self.head_dim, content_energy.dtype))) # [B,N,Q,K]

        dropout = self.dropout if training else 0

        attention = compute_weight_from_energy(energy, x_len,
        ...
        if stream:
            if x_len is None:
                ...
                next_key = layer_utils.get_last_values(k_con, x_len, self.left_mask)
                next_value = layer_utils.get_last_values(v, x_len, self.left_mask)
    ...
    def stream(self, inputs, state, seq_len=None, valid_state=None, training=False):
        #TODO: remove stream() and use call() directly
        return self.call(x=inputs, x_len=seq_len, stream=True, past_state=state, valid_state=valid_state)
...
class AddAttention(tf.keras.layers.Layer):
    ...
    def call(self, query, value, value_lens, key_adv=None, value_adv=None, transpose_value=False):
        if transpose_value:
...
class DotAttention(tf.keras.layers.Layer):
    ...
    def call(self, query, value, value_lens, key_adv=None, value_adv=None, transpose_value=False):
        ...
        query = tf.divide(query, tf.math.sqrt(tf.cast(att_dim_per_head, query.dtype)))

        key_shape = shape_list(key)
        query_shape = shape_list(query)
        value_shape = shape_list(value)
        key = tf.reshape(key, [*key_shape[:2], self.num_att_head, att_dim_per_head]) # [BK,K,N,D]
        query = tf.reshape(query, [*query_shape[:2], self.num_att_head, att_dim_per_head]) # [BQ,Q,N,D]
        ...
        if key.shape[0] == 1 and query.shape[1] == 1:
            ...
            energy = einsum(key, query,
                            trans1=[1,0,2], trans2=[1,0,2], trans3=[0,2,1],
                            transpose_b=True) # [N,BQ,K]
            energy_mask_shape = [1, 1, key_shape[1]] # [1,BK=1,K]
        elif query.shape[0] is not None:
            energy = einsum(query, key, trans1=[0,2,1,3], trans2=[0,2,3,1])
        else:
        ...
        if value_lens is None:
            ...
            if len(value_lens.shape) == 2:
                ...
                mask = tf.sequence_mask(value_lens, maxlen=key_shape[1], dtype=query.dtype)
            mask = tf.reshape(mask, energy_mask_shape)

        att_weight = masked_softmax(energy, mask)

        att = compute_context(att_weight, value, transpose_value=transpose_value)
