...
class SelfAttentionTest(tf.test.TestCase):
    class SampleModel(tf.keras.Model):
        def __init__(self, att_dim, num_head,
                     left_mask, right_mask):
            super().__init__()
            self.att = SelfAttention(att_dim, num_head,
                                     left_mask=left_mask,
    ...
    def test_mask(self):
        ...
        tf.tensor_scatter_nd_update(x, [[1, 4]], tf.random.uniform([1, self.hidden_dim]))
        tf.tensor_scatter_nd_update(x, [[2, 3]], tf.random.uniform([1, self.hidden_dim]))
        tf.tensor_scatter_nd_update(x, [[2, 4]], tf.random.uniform([1, self.hidden_dim]))
...
class RelativeAttentionTest(tf.test.TestCase):
    ...
    def test_gradient(self):
        ...
        def _test_with_num_head(num_head):
            m = SampleModel(self.head_dim * num_head, num_head, self.hidden_dim)
    ...
    def test_rel_shift(self):
        ...
        self.assertAllClose(RelativeAttention.rel_shift(x),
                                      [[[[ 0, 1, 2, 3, 4],
                                         [-1, 0, 1, 2, 3],
                                         [-2,-1, 0, 1, 2],
                                         [-3,-2,-1, 0, 1],
    ...
    def test_relshift_energy(self):
        ...
        self.seq_len = 512
        self.head_dim = 1536
        k_rel = tf.random.uniform([2*self.seq_len-1, num_head, self.head_dim])
        q_rel = tf.random.uniform([batch_size, self.seq_len, num_head, self.head_dim])
...
class UtilTest(tf.test.TestCase):
    def test_sequence_mask(self):
        seq_len = 8
        max_seq = 16
        lengths = tf.random.uniform([seq_len], minval=1, maxval=max_seq, dtype=tf.int32)
        self.assertAllClose(
