...
class LayerTest(unittest.TestCase):
    ...
    def test_weight_share(self):
        ...
        np.testing.assert_array_almost_equal(m.emb.embeddings.numpy(),
                                             m.tied.tied_kernel.numpy())
    ...
    def test_transpose_false(self):
        ...
        class Sample_model(tf.keras.Model):
            ...
            def __init__(self, input_size, hidden_size):
                super(Sample_model, self).__init__()
                self.d1 = tf.keras.layers.Dense(hidden_size, activation='tanh')
        ...
        np.testing.assert_array_almost_equal(m.d1.kernel.numpy(),
                                             m.d2.tied_kernel.numpy())
        np.testing.assert_array_almost_equal(m.d1.bias.numpy(),
                                             m.d2.bias.numpy())
    ...
    def test_dense_op(self):
        dense = tf.keras.layers.Dense(self.hidden_size, activation='tanh')
