...
def calculate_alpha_beta(log_target_probs, target_len, logit_len, log_delta_prob, normalize=True):
    ...
    Args:
        log_target_probs: A tensor of posterior probabilities of each label.
            ...
            Mathematically, it is given by the following equation:
                log (p_{[m]}(y_l | x)).
        target_len: A tensor containing the label lengths.
    ...
    max_target_len = torch.max(target_len)
    ...
    initial_log_beta = (
        (1.0 - torch.nn.functional.one_hot(target_len - 1, max_target_len)) *
        LOG_0)
    ...
    next_log_target_probs = torch.zeros(size=(batch_size, max_target_len))
    ...
    label_mask = seq_loss_util.sequence_mask(
        target_len, max_target_len).unsqueeze(1).to(torch.bool)
    ...
    log_seq_prob_final = seq_loss_util._calculate_unnormalized_log_seq_prob(
        log_alpha, accum_log_alpha_max, logit_len, target_len)
...
#            target_len: The lengths of labels that has the shape of
#                (batch_size).
...
#        #
#        # If target_len < logits_len, then the loss is not valid.
#        invalid_length_mask = (torch.greater_equal(
#            logits_len, target_len)).type(torch.float32)
#
